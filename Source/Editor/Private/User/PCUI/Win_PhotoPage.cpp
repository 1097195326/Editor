...
UWin_PhotoPage::UWin_PhotoPage(const FObjectInitializer& ObjectInitializer) :Super(ObjectInitializer)
{

}

bool UWin_PhotoPage::OnStart()
{
	...
	if (UButton* mBtnPhoto = Cast<UButton>(GetWidgetFromName("Button_Photo")))
	{
		...
		//	拍照按钮--点击事件.
		UUIManager::GetInstance()->RegisterButtonClick(1, m_BtnPhoto, this, &UWin_PhotoPage::PhotoPicture);
	}
	...
	if (UButton* mBtnPicture = Cast<UButton>(GetWidgetFromName("Button_Picture")))
	{
		...
		//	从相册中选择--点击事件.
		UUIManager::GetInstance()->RegisterButtonClick(2, m_BtnPicture, this, &UWin_PhotoPage::PhotoPicture);
	}
	...
	if (UButton* mBtnCancel = Cast<UButton>(GetWidgetFromName("Button_Cancel")))
	{
		...
		//	取消--点击事件.
		UUIManager::GetInstance()->RegisterButtonClick(3, m_BtnCancel, this, &UWin_PhotoPage::PhotoPicture);
	}
...
void UWin_PhotoPage::OnInit()
{

}

void UWin_PhotoPage::OnRelease()
{
...
void UWin_PhotoPage::OnSetParams(int32 _pid, FString & _content, ...)
{

}

void UWin_PhotoPage::OnTick(float InDeltaTimeSecond)
{

}

void UWin_PhotoPage::PhotoPicture(int32 _pid)
{
	...
	if (_pid == 1)
	{
		...
		if(mDesignModule->m_UploadPattern == true)
			IImageMagickModule::Get().OpenCamera(FOnOpenReresult::CreateUObject(this, &UWin_PhotoPage::CBOnOpenPattern));
		else
			IImageMagickModule::Get().OpenCamera(FOnOpenReresult::CreateUObject(this, &UWin_PhotoPage::CBOnOpenPicture));
	...
	if (_pid == 2)
	{
		...
		//打开相册.
		if (mDesignModule->m_UploadPattern == true)
			IImageMagickModule::Get().OpenSelectPattern(FOnOpenReresult::CreateUObject(this, &UWin_PhotoPage::CBOnOpenPattern));
		else
			IImageMagickModule::Get().OpenSelectPattern(FOnOpenReresult::CreateUObject(this, &UWin_PhotoPage::CBOnOpenPicture));
...
void UWin_PhotoPage::GetUploadPattern()
{
	FString mPresentPath = GFileHelper::ProjectPersistentDownloadDir();

	FString mSavePath = FString::Printf(TEXT("%s/customs/"), *mPresentPath);
...
void UWin_PhotoPage::CBOnOpenPattern(bool isSuccess, FString inIconFilePath, FString inFilePath, FVector2D inDPI, FVector2D inRes)
{
	m_DesignModule = GModuleManager::GetInstance()->GetModule<DesignModule>("DesignModule");
...
void UWin_PhotoPage::GetUploadPicture()
{
	IImageMagickModule::Get().OpenSelectPattern(FOnOpenReresult::CreateUObject(this, &UWin_PhotoPage::CBOnOpenPicture));
}

void UWin_PhotoPage::CBOnOpenPicture(bool isSuccess, FString inIconFilePath, FString inFilePath, FVector2D inDPI, FVector2D inRes)
{
	m_DesignModule = GModuleManager::GetInstance()->GetModule<DesignModule>("DesignModule");
