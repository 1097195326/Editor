...
#include "DesignUMG.generated.h"

UCLASS()
class EDITOR_API UDesignUMG : public UBaseUI
{
	GENERATED_BODY()

	UDesignUMG(const FObjectInitializer& ObjectInitializer);
public:
	...
	//设置DetailColor;
	void SetDetailColor(FLinearColor _RGB, UTexture2D * _texture2D);
...
public:
	...
	UUMGSuitComponentDetail *m_ComponentDetail;
	UUMGSuitPartMenu* m_UMG_SuitPartMenu;
	UUMGSuitComponentMenu* m_UMG_SuitComponentMenu;
	UUMGSuitSliceMenu* m_UMG_SuitSliceMenu;
	UDetailPage * m_DetailPage;			//	花型，颜色，面料UI.
