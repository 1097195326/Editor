...
#include "Win_LoginPageCell.generated.h"
 ...
 */
UCLASS()
class EDITOR_API UWin_LoginPageCell : public UBaseUI
{
	GENERATED_BODY()
...
public:
	...
	UWin_LoginPageCell(const FObjectInitializer& ObjectInitializer);
	...
	void HandlePassWordLogin(int32 _pid);

	void HandleCheckNumber();

	void HandleCheckRest(int32 _pid); //重置密码的验证码操作.

	void HandlePassWordReset(int32 _pid); //重置密码的验证码操作.

	void HandleFirstLogin(int32 _pid); //第一次登陸輸入密碼.
	...
	UFUNCTION()
	void LoginBtnCallback(int32 _pid);

	UFUNCTION()
	void GoPagePassWordReset(int32 _pid);
	UFUNCTION()
	void SendChangePage(int32 _pid);
